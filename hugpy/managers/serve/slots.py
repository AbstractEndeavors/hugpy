...
def render_slot_unit(python_bin: str | None = None, user: str | None = None,
    ...
    """A systemd TEMPLATE unit: ``hugpy-slot@.service``.

    The instance number is the slot id (``systemctl enable --now
    hugpy-slot@1``); the agent derives its port from SLOT_ID. Installed
    ...
    return "\n".join((
        "[Unit]",
        "Description=hugpy model slot %i",
        ...
        f"Group={group}",
        *env_lines,
        f"ExecStart={python_bin} -m hugpy.managers.serve.slot_agent",
        "Restart=always",
        "RestartSec=5",
        "TimeoutStopSec=60",
        "",
...
def slot_install_steps(unit_dir: str = "/etc/systemd/system",
    ...
    unit_path = osp.join(unit_dir, "hugpy-slot@.service")
    steps = [("write:" + unit_path, render_slot_unit(main_gpu=main_gpu)),
             ("cmd", "systemctl daemon-reload")]
    for i in range(_slot_count()):
        steps.append(("cmd", f"systemctl enable --now hugpy-slot@{i + 1}"))
    return steps
