...
class labels (see :mod:`hugpy.phone_brick.detector`). This module does the
*reasoning*: it renders those detections as text and hands them to a chat model
via :func:`hugpy.managers.dispatch.execute_prompt`, which produces a natural-
...
RPC shard backend (:mod:`hugpy.phone_brick.rpc_backend`) can lend compute to
