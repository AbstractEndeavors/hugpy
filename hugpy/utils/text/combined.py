...
# wrappers below so `import hugpy` works on a base / phone install
# without them. Install with hugpy[ocr] / [web] to use these.
...
def _image_extractor(path: str) -> str:
    from abstract_ocr import paddle_image          # extra: hugpy[ocr]
    return paddle_image(path)

def _website_extractor(url: str) -> str:
    from abstract_webtools import get_soup_text     # extra: hugpy[web]
...
def _website_body_text(url: str) -> str:
    from abstract_webtools import get_body_from_url  # extra: hugpy[web]
